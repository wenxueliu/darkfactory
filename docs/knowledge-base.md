...
**触发时机：** worktree-controller 报告 `DONE` → sw-controller 验证通过 → sw-knowledge-agent 增量更新
...
```
/sw-controller 重新发现服务           # 全量重新发现
/sw-controller 重新发现 {service-id}  # 仅重新发现指定服务
```
...
> 配置项：`_context/config.yaml` → `sw.kb.freshness.*`
...
不需要全量重新初始化。运行 `/sw-controller 重新发现 {new-service-id}` 即可增量添加，新服务的 KB 文件会自动生成。
