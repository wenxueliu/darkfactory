...
| Agent | Role | Trigger |
|-------|------|---------|
| `sw-controller` | Top-level orchestrator — Intent Gate + Phase Transition + delegation discipline. Routes to specialists, never executes phase work directly. Enhanced with Sisyphus DNA. | 黑灯工厂, orchestration, coordination |
| `sw-tdd-agent` | Autonomous TDD practitioner — RED→GREEN→REFACTOR with "Do NOT Ask" autonomy and TODO obsession. Enhanced with Hephaestus + Sisyphus-Junior DNA. | TDD, unit test, test-first |
...
| Agent | Role | Trigger |
|-------|------|---------|
| `sw-worktree-controller` | Single-task coordinator — drives one task through TDD + review + quality gates | worktree execution, 任务开发 |
...
| `sw-finishing-branch` | Branch completion — 4-option terminal state (merge/PR/keep/discard). Based on Superpowers finishing-a-development-branch. (NEW) | finish branch, 分支收尾, merge, 合并 |
...
│                 │  │                                                 │
│                 │  │   Gate: plan review PASS + grill docs PASS       │
...
│                                                                      │
│   sw-plan-executor (编排层)                                          │
│     └── sw-worktree-controller × N (每任务一个 worktree，并行)       │
│           └── sw-tdd-agent (RED → GREEN → REFACTOR)                 │
│                 ├── sw-lint-checker (跨语言规范检查)                  │
│                 ├── sw-reviewer-logic (P0/P1 correctness bugs)      │
│                 ├── sw-reviewer-security (vulnerabilities)          │
│                 ├── sw-reviewer-performance (bottlenecks)           │
│                 └── sw-reviewer-context (missed requirements)       │
│                                                                      │
│   Feedback loop: sw-receiving-review → pushback protocol            │
│   Debug:   sw-systematic-debugging (根因优先于修复)                  │
│   Verify:  sw-verification-before-completion                         │
│                                                                      │
│   sw-plan-executor: 检查 P0/P1/P2 gate (0 P0, 0 P1)                 │
...
│ Phase 5: merge (合并)                                                 │
│                                                                      │
│   sw-finishing-branch → 4-option terminal state                     │
...
│                                                                      │
│   sw-knowledge-agent → KB 全量更新 (knowledge-update.md)             │
│                                                                      │
│   sw-controller: 检查 delivery-acceptance-gate.md 结果                │
...
以下 Agent 不绑定特定阶段，由 sw-controller 按需在任意阶段水平调用：
