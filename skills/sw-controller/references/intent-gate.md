...
| 类型 | 描述 | 示例 | 路由决策 |
|------|------|------|---------|
| **Trivial** (简单) | 单文件、已知位置、<10 行改动，**且**不涉及跨服务/契约/状态机/并发 | "在 UserController 加个日志" | 直接执行（**必须**口头化意图） |
...
```
收到请求
├─ 是否只需要单文件 + 已知位置 + <10 行，**且** 不涉及跨服务/契约/状态机/并发/错误处理？
│   └─ YES → Trivial → 口头化意图 → 用户确认 → 执行（**Trivial 仍必须通过 Intent Gate**）
├─ 是否有明确的实现动词 (implement/add/create) + 具体范围 (module/endpoint/component)？
│   └─ YES → Explicit → 需求澄清验证（即使表面明确） → 规划 → 委托
├─ 是否以 "解释/如何工作/是什么" 开头？
...
### Trivial 分类的硬约束（避免被滥用为跳过澄清的借口）

满足**全部**以下条件才算 Trivial，缺一不可：

1. **位置已知且唯一** — 改动目标是单文件，且文件路径在调用前可被直接指认
2. **改动量 < 10 行** — 净增/净改行数（不含空行和注释）严格 < 10
3. **不涉及跨服务调用** — 不增加/修改/删除任何 HTTP/gRPC/消息队列契约
4. **不涉及数据库 schema 变更** — 无 DDL、无 migration
5. **不涉及状态机/并发/锁** — 改动不引入新的状态字段、并发原语、事务边界
6. **不改变用户可见行为** — 不影响 API 响应、UI 渲染、错误信息
7. **用户明确指示小改** — 消息中含 "小改/微调/typo/注释/格式/log 字符串" 等信号，**或** 是上一 turn 已确认任务的明确延续

**反模式**: "加个登录按钮" 看起来 Trivial，但实际涉及前端组件 + 后端路由 + 数据库 user 表 + 状态管理 → **不是 Trivial**。"修个 typo" → 是 Trivial。"把日志加一行" → 取决于是否跨服务，但通常算 Trivial。"把函数 f 改成同步" → 涉及并发边界 → **不是 Trivial**。
