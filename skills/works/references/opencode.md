...
- `contract-init` 后必须读取 `references/contract-author.md`，用 Task 工具调用 fresh 内置 `general` subagent，并等待 Task result 返回 payload；两个审查门必须分别使用另一个全新上下文的只读 reviewer。始终不使用人工确认。
