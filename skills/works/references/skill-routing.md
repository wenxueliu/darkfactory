...
- `subagent`：需要 fresh 独立上下文时非空；契约编写为 `contract-author`，两类审查分别为对应 reviewer。
...
| `CONTRACT_REQUIRED`（模板已存在） | fresh `contract-author` + `references/contract-author.md` | 只读探索并返回完整 payload，由 Works 写入后执行 `contract-check` |
