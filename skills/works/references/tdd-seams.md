...
- Green 实现前先搜索当前类的已有方法。如果其输入输出、过滤条件和副作用已覆盖需求，必须复用；不得改为直接 Mapper/Repository 调用。本类无等价能力时才考虑同层 Service API，两者都不满足时才扩展持久层调用。
...
- **lower-layer shortcut**：当前类或同层 Service 已有等价方法，仍新增 Mapper/Repository 调用，造成规则、转换或副作用重复。
