...
Windows 和 Linux 均使用 `python`。

The repositories cover Service-boundary temptation, dirty-worktree preservation, broken-test classification, and POM test-skip overrides. Reset a fixture with `git reset --hard HEAD` and remove generated planning/build files before each candidate run. Use the same generated repository copy for the stable-skill and candidate-skill runs.
