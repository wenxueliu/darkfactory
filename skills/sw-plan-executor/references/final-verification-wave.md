...
从 `{project-root}/_context/config.yaml` 的 `sw.enabled_reviewers` 读取启用的审查者列表。
...
委托 `sw-reviewer-security` 进行安全审查：
...
委托 `sw-reviewer-performance` 进行性能审查：
...
|------|---------------------|------------------------|
| 执行者 | sw-plan-executor (你) | sw-reviewer-* agents |
| 粒度 | 单任务 | 全计划 |
