...
1. [plan-1] Consult sw-pre-planning-consultant for gap analysis (auto-proceed)
...
- 用户确切看到还剩下哪些步骤
- 防止跳过关键步骤（如 sw-pre-planning-consultant 咨询）
- 为每个阶段创建责任感
- 如果会话中断，可以从断点恢复
...
2. plan-1 标记为 in_progress → 咨询 sw-pre-planning-consultant（自动进行，不需要用户确认）
...
## Step 2: sw-pre-planning-consultant 缺口分析（MANDATORY）

**在生成计划之前**，必须调用 sw-pre-planning-consultant 来捕获你可能遗漏的内容:

```
调用 sw-pre-planning-consultant:
"审查本次规划会话，在我生成工作计划之前:
...
### sw-pre-planning-consultant 调用规则
...
### NEVER
- 跳过低谷分析（sw-pre-planning-consultant）
- 多次 Write 到同一计划文件
- 一次性生成所有任务（超出输出限制）
- 不追加以验证追加完整性
- 跳过自审查
...
### ALWAYS
- 触发后立即注册 TodoWrite
- 先咨询 sw-pre-planning-consultant
- 骨架一次写入，任务分批追加
- 每批后验证完整性
- 自审查后分类缺口
- 单一计划文件包含所有内容
