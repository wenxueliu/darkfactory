...
### 何时必须委托 hw-strategic-advisor

以下情况，在 Planner Directives 中 RECOMMEND 后续委托 `hw-strategic-advisor` 深度审查:
...
如果触发顾问咨询，确保提供:
```
Delegate to hw-strategic-advisor with:
- Context: [当前架构决策的上下文]
- Options: [考虑的 2-3 个方案]
- Constraints: [已知约束]
...
- 存储于: `{project-root}/_bmad/memory/hw-shared/design-decisions.md`
- 每个决策独立一节，按时间倒序排列
- 使用一致的格式使决策可搜索、可比较
...
```
SHOULD delegate deep architecture analysis to hw-strategic-advisor when:
  - 3+ systems interact
  - Security/performance trade-offs exist
  - Unfamiliar tech stack involved
SHOULD store decision records in {project-root}/_bmad/memory/hw-shared/design-decisions.md.
...
1. **[Check]** 决策记录完整:
   ```
   验证命令: grep -c "## Decision:" {project-root}/_bmad/memory/hw-shared/design-decisions.md
