...
**委托给 `hw-codebase-explorer`**，具体搜索指令:
...
**委托给 `hw-external-researcher`**，搜索指令基于请求内容:
...
MUST follow the project structure and conventions discovered by hw-codebase-explorer.
...
3. **[Static Analysis]** 项目结构合规:
   ```
   验证: 项目结构匹配 hw-codebase-explorer 发现的 monorepo/组织约定
